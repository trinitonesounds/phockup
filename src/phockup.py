...
class Phockup:
    ...
    def process_file(self, filename):
        ...
        """
        if (str.endswith(filename, '.xmp')) or (str.endswith(filename, '.bib')):
            return None
...
but looking for '{self.file_type}'"
            ...
            if os.path.isfile(target_file):
                ...
                self.process_bib(filename, target_file_name, suffix, output)
                break
    ...
    def process_bib(self, original_filename, file_name, suffix, output):
        """
        Process bib files. These are metadata for RAW images
        """
        bib_original_with_ext = original_filename + '.bib'
        bib_original_without_ext = os.path.splitext(original_filename)[0] + '.bib'
        ...
        for original, target in bib_files.items():
            ...
            logger.info(f'{original} => {bib_path}')
